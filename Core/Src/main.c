...
static uint8_t rxByte;   // ✅ reçoit 1 octet UART à la fois

volatile uint32_t uart6RxCount = 0;

volatile uint32_t g_soil_adc[4]     = {0,0,0,0};  // 0..4095
volatile uint32_t g_soil_percent[4] = {0,0,0,0};  // 0..100

volatile int32_t g_temp_c_x10 = 0;   // ex: 234 = 23.4°C
...
#define HUM_ADC_DRY   3200  // valeur ADC capteur "sec" (à l’air / terre sèche)
#define HUM_ADC_WET   1400  // valeur ADC capteur "mouillé" (dans l’eau)
...
// Retourne 0..100 (% humidité)
static uint32_t adc_to_percent(uint32_t adc)
{
    // évite division par 0
    if (HUM_ADC_DRY == HUM_ADC_WET) return 0;

    // cas classique : SEC = ADC haut, MOUILLE = ADC bas
    // percent = (DRY - adc) / (DRY - WET) * 100
    int32_t p = (int32_t)( (int32_t)(HUM_ADC_DRY - adc) * 100 / (int32_t)(HUM_ADC_DRY - HUM_ADC_WET) );
...
void StartDefaultTask(void *argument)
{
  ...
  // (Optionnel) petite pause au boot
  osDelay(500);
  ...
  sht3x_t sht;
  sht.hi2c = &hi2c1;
  sht.addr_7bit = 0x44; // <- change en 0x45 si ADDR=VCC

  // init capteur (si fail on continue mais on peut debug via UART)
  HAL_StatusTypeDef sht_ok = SHT3X_Init(&sht);

  bh1750_t bh;
  // ADDR à GND => 0x23 (addrPinToVcc = 0)
  HAL_StatusTypeDef bh_ok = BH1750_Init(&bh, &hi2c1, 0);


  for(;;)
  {
	  // 1) Lance la séquence de 4 conversions (scan)
	  HAL_ADC_Start(&hadc3);

	  // 2) Lire les 4 rangs (Rank1..Rank4) dans l'ordre
	  for (int i = 0; i < 4; i++)
	  {
	      if (HAL_ADC_PollForConversion(&hadc3, 20) == HAL_OK)
	      {
	          uint32_t adc = HAL_ADC_GetValue(&hadc3);   // valeur du rang i+1
	          g_soil_adc[i] = adc;
	          g_soil_percent[i] = adc_to_percent(adc);
	      }
	  }
	  ...
	  // (option) envoi UART debug
	  int len = snprintf(msg, sizeof(msg),
	      "SOIL1=%lu;ADC1=%lu | SOIL2=%lu;ADC2=%lu | SOIL3=%lu;ADC3=%lu | SOIL4=%lu;ADC4=%lu\n",
	      (unsigned long)g_soil_percent[0], (unsigned long)g_soil_adc[0],
	      (unsigned long)g_soil_percent[1], (unsigned long)g_soil_adc[1],
	      (unsigned long)g_soil_percent[2], (unsigned long)g_soil_adc[2],
	      (unsigned long)g_soil_percent[3], (unsigned long)g_soil_adc[3]
	  );
	  HAL_UART_Transmit(&huart6, (uint8_t*)msg, len, 100);

    // --- Lecture SHT3x (temp + humidité air) ---
    float tC = 0.0f, rh = 0.0f;
    if (SHT3X_ReadTempHum(&sht, &tC, &rh) == HAL_OK)
    {
        g_temp_c_x10 = (int32_t)(tC * 10.0f);
        g_air_rh_x10 = (uint32_t)(rh * 10.0f); // déjà prêt pour plus tard

        // --- Envoi UART AIR (temp + humidité air) ---
        int t_int = (int)(g_temp_c_x10 / 10);
        int t_dec = (int)(g_temp_c_x10 % 10);
        unsigned long rh_int = (unsigned long)(g_air_rh_x10 / 10);
        unsigned long rh_dec = (unsigned long)(g_air_rh_x10 % 10);

        int len = snprintf(msg, sizeof(msg), "TEMP=%d.%d;AIRH=%lu.%lu\n", t_int, t_dec, rh_int, rh_dec);
        HAL_UART_Transmit(&huart6, (uint8_t*)msg, len, 100);
    ...
    // --- Lecture BH1750 (LUX) ---
    uint32_t lux = 0;
    if (bh_ok == HAL_OK && BH1750_ReadLux(&bh, &lux) == HAL_OK)
    {
        g_lux = lux;
