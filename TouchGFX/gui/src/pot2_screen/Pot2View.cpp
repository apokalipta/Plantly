...
static void format_arrosage(uint8_t pot, touchgfx::Unicode::UnicodeChar* buf, uint16_t bufSize)
{
...
void Pot2View::setupScreen()
{
    ...
    char name[33] = {0};
    if (Plantly_PotName_Get(1, name, sizeof(name)) && name[0])
        touchgfx::Unicode::fromUTF8((uint8_t*)name, TitreBuffer, TITRE_SIZE);
    else
        touchgfx::Unicode::snprintf(TitreBuffer, TITRE_SIZE, "POT 2");
    Titre.invalidate();
    ...
    char espece[33] = {0};
    if (Plantly_Espece_Get(1, espece, sizeof(espece)) && espece[0])
    ...
    format_arrosage(1, DateArrosageBuffer, DATEARROSAGE_SIZE);
    ...
    if (!Plantly_PotName_Get(1, lastTitleName, sizeof(lastTitleName)))
        std::strncpy(lastTitleName, "POT 2", sizeof(lastTitleName) - 1);
}
...
void Pot2View::handleTickEvent()
{
    ...
    uint32_t soilPct = g_soil_percent[1];
...
void Pot2View::updateIdleVideo()
{
    ...
    if (app.isIdleVideoActive())
    {
        if (!videoShown)
        {
            ...
            if (hum < 30) { chaud.setVisible(true); chaud.startAnimation(true,false,true); chaud.invalidate(); }
            else if (hum <= 70) { contente.setVisible(true); contente.startAnimation(true,false,true); contente.invalidate(); }
            else { froid.setVisible(true); froid.startAnimation(true,false,true); froid.invalidate(); }
        }
    }
    else
    {
...
void Pot2View::ResetEau()
{
    Plantly_Arrosage_Clear(1);
    format_arrosage(1, DateArrosageBuffer, DATEARROSAGE_SIZE);
    DateArrosage.invalidate();
    Plantly_Arrosage_Send(1);
}
